...
import { leerTag, resumir, ficha } from '../../../lib/aspirante';
...
export async function POST(request) {
  ...
  if (!PERMITIDOS.includes(String(chatId))) {
    ...
    if (msg.chat?.type === 'private' && texto) {
      const r = await flujoSolicitud(msg, texto);
...
//
// El clan se queda en "solo por invitacion" -que es lo correcto- y en la
// descripcion va el @usuario del bot. Quien quiera entrar habla con
// Heraldo, y los lideres deciden con los numeros delante.

/** El grupo de la comunidad: el id negativo de la lista blanca. */
const GRUPO = PERMITIDOS.find((x) => x.startsWith('-')) ?? PERMITIDOS[0];
...
/**
 * Conversacion con alguien de fuera. Devuelve el texto a contestar, o
 * null para no contestar nada.
 *
 ...
 */
async function flujoSolicitud(msg, texto) {
  ...
  if (!sol) {
    ...
    const { data: atado } = await admin
      ...
      .eq('tg_user_id', uid)
      .maybeSingle();
    if (atado || (await esLider(GRUPO, uid))) {
 ...
 *
 * Solo llega a quien le haya dado alguna vez a "Empezar" en el bot:
...
async function avisarLideres(sol) {
  const res = await fetch(
    `https://api.telegram.org/bot${TOKEN}/getChatAdministrators?chat_id=${GRUPO}`
  );
  ...
  for (const a of admins) {
    await responder(a.user.id, aviso).catch(() => {});
  }
}
